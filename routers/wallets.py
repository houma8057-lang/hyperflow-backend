...
async def diag_rsi_failure_swing_backtest(
    ...
    using only price data on or before each checkpoint date. Safe to
    ...
    while current <= end:
        ...
        results.append({
            ...
            "bearish_days_since_break": bearish.get("days_since_break"),
            "bullish_stage": bullish.get("stage"),
            "bullish_break_date": bullish.get("break_date"),
            "bullish_days_since_break": bullish.get("days_since_break"),
        })
        current += timedelta(days=step_days)
